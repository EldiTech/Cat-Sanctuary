...
function isRecoveryRoom(cat: CatRecord) {
  const roomValue = String(cat.assignedRoom ?? cat.room ?? '').toLowerCase();
  return roomValue.includes('recovery');
}

export default function RecoveryRoomScreen({ onBack }: RecoveryRoomScreenProps) {
    ...
    <SafeAreaView style={styles.screen} edges={['top', 'left', 'right']}>
      ...
      >
        <View style={styles.headerRow}>
          <Text style={styles.headerTitle}>Recovery Room</Text>
        ...
        <ScrollView contentContainerStyle={styles.list} showsVerticalScrollIndicator={false}>
          {residentNames.length === 0 ? (
            <View style={styles.emptyCard}>
              ...
              <Text style={styles.emptySubtitle}>Assign cats with a room value containing Recovery.</Text>
            </View>
          ) : (
